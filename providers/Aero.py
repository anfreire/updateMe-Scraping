...
class Aero(Selenium):
	...
	def remove_adblock(self):
		h2 = self.find_elements(By.XPATH, "//h2")
		found = None
		for h in h2:
	...
	def get_href_by_text(self, text: str) -> str | None:
		elememts = self.find_elements(By.TAG_NAME, "a")
		for el in elememts:
			if el.text == text and el.get_attribute("href"):
				return el.get_attribute("href")

	def click_span(self, class_attr: str):
		spans = self.find_elements(By.TAG_NAME, "span")
		for span in spans:
	...
	def get_href_by_ending_link(self, text: str) -> str | None:
		elememts = self.find_elements(By.XPATH, "//a[@href]")
		for el in elememts:
